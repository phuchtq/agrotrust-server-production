...
func InitializePendingWithdrawProposalRoute(server *gin.Engine) {
	...
	// Auth group
	var managerGroup = server.Group(contextPath, middleware.Authorize, middleware.ManagerRoleAuthorize)
	managerGroup.GET("/:id", transport.GetPendingWithdrawProposal)
	managerGroup.GET("", transport.GetPendingWithdrawProposals)
	managerGroup.POST("", transport.CreatePendingWithdrawProposal)

	var adminGroup = server.Group(contextPath, middleware.Authorize, middleware.AdminAuthorize)
	adminGroup.POST("/:id/approve", transport.ApprovePendingWithdrawProposal)
	adminGroup.POST("/:id/refuse", transport.RefusePendingWithdrawProposal)
}
